...
class Solution {
    public int maxNonOverlappingOddPalindromeSum(String s) {
        int n = s.length();
        if (n < 2) {
            return 0;
        }

        int[] maxLenEndingHere = new int[n];
        int[] maxLenStartingHere = new int[n];
        ...
        for (int i = 0; i < n; i++) {
            ...
            while (l >= 0 && r < n && s.charAt(l) == s.charAt(r)) {
                int len = r - l + 1;
                if (len % 2 != 0) {
                    maxLenEndingHere[r] = Math.max(maxLenEndingHere[r], len);
                    maxLenStartingHere[l] = Math.max(maxLenStartingHere[l], len);
                }
                l--;
                r++;
            }
        ...
        for (int i = 1; i < n; i++) {
            maxLenEndingHere[i] = Math.max(maxLenEndingHere[i], maxLenEndingHere[i - 1]);
        }
        
        for (int i = n - 2; i >= 0; i--) {
            maxLenStartingHere[i] = Math.max(maxLenStartingHere[i], maxLenStartingHere[i + 1]);
        }
        
        int maxSum = 0;
        for (int i = 0; i < n - 1; i++) {
            if (maxLenEndingHere[i] > 0 && maxLenStartingHere[i + 1] > 0) {
                maxSum = Math.max(maxSum, maxLenEndingHere[i] + maxLenStartingHere[i + 1]);
            }
        }
