...
class Solution {
    public long solve(String[] arr) {
        int[] diff = new int[arr.length];
        for (int i = 0; i < arr.length; i++) {
            diff[i] = countDifference(arr[i]);
        }
        ...
        for (int d : diff) {
            ...
            if (sumFreq.containsKey(prefixSum)) {
                count += sumFreq.get(prefixSum);
                sumFreq.put(prefixSum, sumFreq.get(prefixSum) + 1);
            } else {
                sumFreq.put(prefixSum, 1);
            }
        }
    ...
    private int countDifference(String s) {
        int vowels = 0;
