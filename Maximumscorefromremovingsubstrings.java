...
class Solution {
    public int maximumGain(String s, int x, int y) {
        int totalScore = 0;
        String s1, s2; // s1 for the higher-scoring pair, s2 for the lower-scoring pair
        int score1, score2;
        ...
        sb = new StringBuilder(); // Reset StringBuilder for the second pass

        for (char c : remainingString.toCharArray()) {
            if (c == s2.charAt(1) && sb.length() > 0 && sb.charAt(sb.length() - 1) == s2.charAt(0)) {
                sb.deleteCharAt(sb.length() - 1);
                totalScore += score2;
            } else {
                sb.append(c);
