 ...
 */
class Solution {
    public boolean isPossible(String pat, String tar) {
        int n = pat.length();
        int m = tar.length();
        
        // A necessary condition is that tar's length cannot be greater than pat's.
        if (m > n) {
            return false;
        }
        ...
        // Counter for characters in `pat` that are not part of `tar`.
        int unmatchedCount = 0;

        // Iterate through `pat` with pointer `i`.
        for (int i = 0; i < n; i++) {
            // Greedily check if `pat[i]` matches the next character of `tar`.
            if (j < m && pat.charAt(i) == tar.charAt(j)) {
                j++; // Match found, advance `tar` pointer. This character is "appended".
            } else {
                // No match. This character from `pat` must be part of a pair of operations
                // (append and then later delete).
                unmatchedCount++;
            }
        }
        
        // After iterating through all of `pat`, we must have matched all of `tar`.
        // If not, it's impossible.
        if (j != m) {
            return false;
        }
        
        // The remaining `unmatchedCount` characters must have been appended and then deleted.
        // Each such pair of operations consumes one character from `pat` for the append and one for the delete.
        // This means `unmatchedCount` must be even.
        // If `unmatchedCount` is odd, it's impossible to balance the appends and deletes.
        return unmatchedCount % 2 == 0;
    }
}