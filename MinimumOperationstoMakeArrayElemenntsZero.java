...
class Solution {
    private java.util.Map<Long, Long> memoF = new java.util.HashMap<>();
    private java.util.Map<Long, Long> memoOps = new java.util.HashMap<>();

    private long getOps(long x) {
        if (x == 0) {
            return 0;
        }
        if (memoOps.containsKey(x)) {
            return memoOps.get(x);
        }
        long result = 1 + getOps(x / 4);
        memoOps.put(x, result);
        return result;
    }
    ...
    private long calculateF(long n) {
        ...
        long m = n / 4;
        long opsM = getOps(m);
        long result = n + 4 * calculateF(m) + (n % 4 - 3) * opsM;
        memoF.put(n, result);
        return result;
    }
    ...
    public long minimumOperations(int[][] queries) {
        ...
        for (int[] query : queries) {
            long l = query[0];
            long r = query[1];
            long sumOpsLToR = calculateF(r) - calculateF(l - 1);
            totalSum += sumOpsLToR / 2;
        }
        return totalSum;
    }
}